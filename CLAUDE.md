...
- 발행 순서: `POST /api/v1/media`(MinIO 저장) → 반환 id를 본문/cover에 연결 → `POST /api/v1/posts`(또는 portfolio/docs/services/announcements).
- 본문은 Plate JSON, 또는 `src/lib/markdown-to-plate.ts`로 마크다운 전송.
- 반복 워크플로는 `/publish-content` 스킬 사용. 추후 이 REST 위에 얇은 MCP 서버를 얹는다.
