...
const sections = [
  {
    ...
    heading: '개요 · OVERVIEW',
    body:
      '여러 각도를 찍으려면 각도만큼 장비를 사야 했고, 그걸 방송으로 내보내려면 OBS를 따로 깔아 씬을 잡아야 했습니다. ' +
      '그런데 서랍에는 이미 안 쓰는 폰이 두 대 있었습니다. GHC는 그 기기들을 그대로 카메라로 씁니다. ' +
      '같은 계정으로 로그인만 하면 카메라 목록에 자동 등록되고, PC에서 방에 들어가면 내 온라인 기기들이 자동으로 호출되어 카메라만 올립니다. ' +
      '서버는 클라우드가 아니라 집에 있는 컴퓨터 한 대입니다.',
    bullets: [
      '장비를 새로 사지 않는다 — 안 쓰는 폰이 곧 카메라. 기기 수만큼 앵글이 늘어난다',
      '폰을 만질 필요가 없다 — 삼각대에 올려두면 PC에서 켜고 끄고 렌즈까지 원격으로 바꾼다',
      'OBS 없이 송출된다 — 데스크탑 앱이 OS 네이티브 API로 화면·앱 오디오·DRM 재생 화면까지 캡처한다',
  ...
  {
    kind: 'gallery',
    heading: 'OBS 없이 송출하기 · LIVE',
    body: '방마다 RTMP 주소와 스트림 키가 발급돼 OBS가 그대로 합류할 수 있고, 데스크탑 앱을 쓰면 OBS 자체가 필요 없습니다.',
    media: [
      { kind: 'video', mediaId: M.vRtmp, caption: '좌: 방장의 송출 설정 / 우: 다른 계정 시청자 — RTMP가 Ingress를 거쳐 방 트랙이 된다', poster: pRtmp },
      { kind: 'image', mediaId: M.rtmpSetup, caption: '라이브 송출 설정 (스트림 키는 마스킹)' },
    ],
  },
  {
    kind: 'features',
    ...
    bullets: [
      ...
      'RTMP 인입 — 방마다 주소와 키를 발급해 OBS가 그대로 합류',
      '네이티브 캡처 — 데스크탑 앱이 창·화면을 OS 네이티브 API로 캡처해 송출. OBS 대체',
      '앱별 오디오 캡처 — 특정 앱 소리만 방송에 넣고 내 스피커에서는 끌 수 있다',
      '브라우저 라이브 — DRM 재생이 되는 전용 창을 띄워 그 창을 송출',
...
const summary =
  ...
  '여러 기기를 독립 카메라로 등록해 멀티앵글로 송출하는 셀프호스팅 화상회의·라이브 서비스.'
